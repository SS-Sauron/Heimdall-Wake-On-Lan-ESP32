...
7.  storage_is_provisioned()           Check for saved credentials in NVS
    ...
    │                       Blocks until credentials saved → esp_restart()
    │
...
4. On submission, the portal validates all fields server-side (RFC 1123 for hostname, URL format for MQTT)
5. Credentials written to NVS via `storage_save_credentials()`
6. If OPSEC is enabled, HMAC secret and TOTP seed are generated now and returned in the JSON success response — **this is the only moment they are visible**
7. 500 ms delay (HTTP response flush) → `esp_restart()`

**HTTP server:** 9 URI handlers registered on port 80. `max_uri_handlers` set to 12 for headroom. The wildcard catch-all (`/*`) is registered last and handles all OS captive-portal detection URLs.
...
6. `esp_mqtt_client_init()` + `esp_mqtt_client_start()`
...
**HARDENED** — Enable by setting `CONFIG_OPSEC_HARDENED=y` or individually enabling the OPSEC Kconfig options. All four OPSEC features activate:
...
| `CONFIG_WIFI_STA_MAX_RETRY` | 5 | Wrong-credential strikes before portal fallback |
| `CONFIG_WIFI_STA_RETRY_DELAY_MS` | 1000 | Delay between reconnect attempts |
| `CONFIG_WIFI_STA_CONNECT_TIMEOUT_MS` | 30000 | Initial connection timeout |
...
| `CONFIG_PORTAL_TIMEOUT_SEC` | 0 | Portal timeout (0 = wait forever) |
...
| `mn` | string | MQTT username |
| `mk` | string | MQTT password |
...
Keys `hs` and `ts` are generated once during provisioning and never transmitted after that point. They are displayed to the user in the provisioning success response and never again — if lost, they can only be recovered by factory-resetting and reprovisioning.
